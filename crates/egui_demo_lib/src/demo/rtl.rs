...
/// A focused demonstration of bidirectional text layout and editing.
...
impl RtlDemo {
    ...
    fn static_samples(ui: &mut egui::Ui) {
        ...
        ui.label(
            "These samples use embedded Arabic and Hebrew fixtures plus egui's bundled \
             Ubuntu font. The family and bytes are identical on native and wasm.",
        );
    ...
    fn layout_samples(&mut self, ui: &mut egui::Ui) {
        ...
        let mut styled = LayoutJob {
            wrap: TextWrapping {
                max_width: 310.0,
                ..Default::default()
            },
            halign: self.physical_alignment,
            ..Default::default()
        };
        for (text, color) in [
        ...
        ui.add(egui::Label::new(RichText::new("بِ بِ بِ — שָׁלוֹם שָׁלוֹם").font(rtl_font(20.0))).wrap());
...
impl crate::View for RtlDemo {
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "This page exercises bidirectional text layout and editing, including known limitations. It does not claim complete RTL support.",
        );
        ui.separator();
